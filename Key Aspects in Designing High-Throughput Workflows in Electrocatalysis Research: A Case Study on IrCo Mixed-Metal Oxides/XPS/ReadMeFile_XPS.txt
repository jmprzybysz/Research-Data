____________________Key Aspects in Designing High-Throughput Workflows in Electrocatalysis Research: A Case Study on IrCo Mixed-Metal Oxides____________________

Last updated: 2024-21-06

______Contact______
* Joanna Przybysz
* j.przybysz@fz-juelich.de
* +49 9131-12538187
* ORCID: 0009-0000-8402-5691
* Helmholtz Institute Erlangen-Nürnberg for Renewable Energy
* Cauerstraße 1, 91058 Erlangen, Germany 

______Licence______
*Dataset for "Key Aspects in Designing High-Throughput Workflows in Electrocatalysis Research: A Case Study on IrCo Mixed-Metal Oxides" © 2024 by Joanna Przybysz is licensed under CC BY
*licence information: https://creativecommons.org/licenses/by-nc-sa/4.0/?ref=chooser-v1

------------------------------------------------------------------------------------------------

______About the dataset______
XPS analysis was performed by: 
* Tomáš Hrbek
* ORCID: 0000-0002-6041-2125
* Department of Surface and Plasma Science, Faculty of Mathematics and Physics Charles University
* V Holešovičkách 2, 180 00 Prague 8, Czech Republic 

With the growing interest of the electrochemical community in high-throughput (HT) experimentation as a powerful tool in accelerating materials discovery, the implementation of HT methodologies and the design of HT workflows has gained traction. We identify 6 aspects essential to HT workflow design in electrochemistry and beyond to ease the incorporation of HT methods in the community’s research and to assist in their improvement. We study IrCo mixed-metal oxides (MMOs) for the oxygen evolution reaction (OER) in acidic media using the mentioned aspects to provide a practical example of possible workflow design pitfalls and strategies to counteract them. 

______Methods of data collection______
XPS spectra:
Conventional XPS system (SPECS Surface Nano Analysis, GmbH Germany) with ultrahigh vacuum chamber equipped with a monochromated Al Kα X-ray source (hν = 1486.6 eV) and a multichannel electron energy analyzer (SPECS Phoibos 150) with 1D-DLD detector.

______Methods of data processing______
The data were corrected on the binding energy of C 1s to 284.2 eV.

------------------------------------------------------------------------------------------------
______File formats______
XPS spectra - original files exported to CSV

______Abbreviations______
XPS	X-ray photoelectron spectroscopy
BE	Binding energy
AP	As prepared
ET	Electrochemically treated

______Units______
All XPS spectra are in binding energy (eV) vs. intensity (cps)

------------------------------------------------------------------------------------------------
______List of files______

XPS spectra of samples:
Co_spectra_before.dat XPS spectra of Co from AP samples
Ir_spectra_before.dat XPS spectra of Ir from ET samples
Co_spectra_after.dat XPS spectra of Co from AP samples
Ir_spectra_after.dat XPS spectra of Ir from ET samples
